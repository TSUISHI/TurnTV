...
func enter_select_mode() -> void:
	...
	capture_screen = win.current_screen
	...
	win.position = DisplayServer.screen_get_position(capture_screen)
	...
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT:
		if event.pressed:
			...
			if local_rect.size.x >= MIN_SELECT_SIZE and local_rect.size.y >= MIN_SELECT_SIZE:
				var screen_origin: Vector2i = DisplayServer.screen_get_position(capture_screen)
				capture_rect = Rect2i(screen_origin + Vector2i(local_rect.position), Vector2i(local_rect.size))
				enter_tv_mode(true)
			else:
				queue_redraw()
...
func _update_edge_cursor(pos: Vector2) -> void:
	...
	if edge == DisplayServer.WINDOW_EDGE_LEFT or edge == DisplayServer.WINDOW_EDGE_RIGHT:
...
func _draw_select_overlay() -> void:
	...
	if selecting:
		...
		var size_text: String = "%d x %d" % [int(r.size.x), int(r.size.y)]
